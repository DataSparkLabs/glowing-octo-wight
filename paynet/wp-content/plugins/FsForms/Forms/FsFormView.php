...
class FsFormView
{
	...
	extract( shortcode_atts( array(
        ...
        if($schema->calendar == null)
        {
            $html .= $this->render_form($schema, $meta, $privacy_url, $submit_label, $schema->redirect);
        }
        else
        {
            $html .= '<div class="fs-scheduler-container" data-calendar="'.$schema->calendar->recnum.'">
                ...
                $html .= $this->render_scheduler_form($schema, $meta, $privacy_url, $submit_label, $schema->redirect);
                
            $html.='</div>';
        }
        return $html;
    }
    private function render_form($schema, $meta, $privacy_url, $submit_label, $redirect)
    {
    ...
    private function render_scheduler_form($schema, $meta, $privacy_url, $submit_label, $redirect)
    {
        global $fs_form_view_values;
        $html = '<div id="fs-form-container-'.$schema->recnum.'" class="fs-form-container">';               
        $html .= '<form id="fs-form-'.$schema->recnum.'" action="'.admin_url( 'admin-ajax.php' ).'" method="post" class="fs-form" data-form="'.$schema->recnum.'" enctype="multipart/form-data">
    ...
    private function render_field($field, $value=null)
    {
        ...
        if($value != null)
        {
            $html .= '<input type="hidden" name="'.$field->name.'" value="'.$value->value.'" />';
        }        
        else
        {
            switch($field->type)
            {
                case self::$TextInputType:
                    $html = $this->render_text_input_field($field, $value);
                    break;
                case self::$TextAreaType:
                    $html = $this->render_textarea_field($field, $value);
                    break;
                case self::$CheckInputsType:
                    $html = $this->render_checks_input_field($field, $value);
                    break;
                case self::$RadioInputsType:
                    $html = $this->render_radios_input_field($field, $value);
                    break;
                case self::$DateInputType:
                    $html = $this->render_date_input_field($field, $value);
                    break;
            }
        }
        return $html;
    ...
    public function process_form()
    {
        ...
        try
        {
            ...
            ?><span class="result-success">Form Submited!</span><?php
        } 
        catch (Exception $ex)
        {
            echo $ex->getMessage();
